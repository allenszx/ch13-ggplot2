...
plot_2 <- ggplot(data = diamonds_sample) +
  geom_point(mapping = aes(x = carat, y = price), color = "blue", size = 1)

# Draw a scatter plot for `diamonds_sample` of `price` by `carat`, where each
# point has an aesthetic _shape_ based on the diamond's `cut`.

plot_3 <- ggplot(data = diamonds_sample) +
  geom_point(mapping = aes(x = carat, y = price, shape = cut), color = "blue", size = 1)

# Draw a scatter plot for `diamonds_sample` of *`cut`* by `carat`, where each
# point has an aesthetic _size_ based on the diamond's *`price`*
plot_4 <- ggplot(data = diamonds_sample) +
  geom_point(mapping = aes(x = cut, y = carat, size = price), color = "blue")

# Try coloring the above plot based on the diamond's price!
plot_5 <- ggplot(data = diamonds_sample) +
